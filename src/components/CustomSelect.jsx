...
export function CustomSelect({ value, onChange, options, placeholder = 'Selecione...', showTempAdd = false }) {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 flex flex-col py-1 animate-fadeIn">
          
          <div className="px-2 pb-2 mt-1 sticky top-0 bg-white z-10 border-b border-slate-100">
            <input
              type="text"
          ...
          <div className="overflow-y-auto">
            ...
            {options.filter(opt => {
              ...
              <div className="px-4 py-3 text-sm text-slate-500 text-center">Nenhum resultado encontrado.</div>
            )}
          </div>
        </div>
      )}
